...
void eval(char *cmdline) 
{
    ...
    if (!builtin_cmd(argv)){
        ...
        sigemptyset(&mask);//make an empty set
        sigaddset(&mask, SIGCHLD);
        /*Block SIGCHLD and save previous blocked set */
        sigprocmask(SIG_BLOCK, &mask, &prev_mask);
        if ((pid = fork()) == 0) {  /* Child runs user job */
            sigprocmask(SIG_SETMASK, &prev_mask, NULL);//unblock. null means we don't save the (was)current masked signal.
            setpgid(0, 0);//puts the child in a new process group where         group ID== child's PID
...
void do_bgfg(char **argv) 
{
    ...
    if (!strncmp(argv[0], "bg", 2)){
        if (!strncmp(argv[1],"%",1)){
            ...
            return;}
            getjobjid(jobs,atoi(argv[1]+1))->state = 2;
            pid =getjobjid(jobs,atoi(argv[1]+1))->pid;//function atoi converts string to integer!
        ...
        kill(-pid, SIGCONT);}
    if (!strncmp(argv[0],"fg",2)){//when user wants to run in 'fg'
        if (!strncmp(argv[1],"%",1)){
            if (getjobjid(jobs,atoi(argv[1]+1))==NULL)
            {printf("%s: No such job\n",argv[1]);
            return;}
            getjobjid(jobs,atoi(argv[1]+1))->state = 1;
...
void sigchld_handler(int sig) 
{
 //  printf("passed   SIGCHLD HANDLER \n");
    int status;
    pid_t pid;
    pid = waitpid(fgpid(jobs), &status, WUNTRACED);
    int jid;
    jid= pid2jid(fgpid(jobs));
    if(pid < 0){
       return;}
